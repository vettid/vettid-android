...
**Last Updated:** 2026-01-01 16:25 UTC (🐛 connection.create-invite handler bug)

---

## 🐛 CURRENT ISSUE: connection.create-invite Handler Bug

**Priority:** High
**Status:** ⏳ Needs Backend Fix

**Problem:** The `connection.create-invite` handler incorrectly requires a `connection_id` field.
...
**Error Response:**
```
HANDLER_ERROR: connection_id is required
```

**Expected Behavior:** The handler should:
1. Generate a new `connection_id` (e.g., `conn-xxx`)
2. Create NATS credentials for the peer
3. Return the connection details including the generated `connection_id`

**Per API-STATUS.md Documentation:**
```json
// Request format (per docs)
{
...
// Expected response
{
  "connection_id": "conn-xyz...",  // Generated by handler
...
**Affected File:** `vault-manager/internal/handlers/connection/create_invite.go`
